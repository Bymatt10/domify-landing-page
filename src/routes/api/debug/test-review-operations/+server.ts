...
export const GET: RequestHandler = async ({ url }) => {
	try {
		...
		// Verificar si el usuario específico tiene reseñas
		let userReviews = [];
		if (userId) {
			const { data: userReviewsData, error: userReviewsError } = await supabaseAdmin
				.from('reviews')
				.select('*')
				.eq('provider_profile_id', providerId)
			...
			if (!userReviewsError && userReviewsData) {
		...
		return json({
			...
			data: {
				provider,
				allReviews: reviewsWithCustomers,
				totalReviews: reviewsWithCustomers.length,
				userReviews: userReviews,
				userReviewCount: userReviews.length,
				canAddMoreReviews: true, // Ahora permitimos múltiples reseñas
				features: {
					multipleReviews: true,
					editReviews: true,
					deleteReviews: true,
					updatedAtTracking: true
				}
			}
		});

	} catch (error) {
		console.error('Error in test-review-operations:', error);
